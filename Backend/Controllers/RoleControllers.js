...
const getRoles = asyncHandler(async (req , res) => {
    try {
        ...
        console.log(error);
        return res.status(500).json({ err:error , message:"Some error occur while get the roles"});
    }
});
...
const addRole = asyncHandler(async (req , res) => {
    ...
    const roleExits = await Role.findOne({where:{Role_id:Role_id}})
    if(roleExits){
        return res.status(400).json({message:"oops ! Sir Role is Already Exits"});
    }
    ...
    try {
        ...
        console.log(error);
        return res.status(500).json({message:"Some errors is occur While adding the role"});
    }
});
